...
PhysicsObject::PhysicsObject() {
	...
	Rotation = Quaternion(vec3(1, 1, 1), 0);
	Mass = 1.0f;
...
void PhysicsObject::HandleCollision(TriangleMeshCollider& collider, float deltaT) {
	...
	desiredDeltaVelocity *= -1.0f;

	// desiredDeltaVelocity = 13.0f;
...
void PhysicsObject::UpdateMatrix() {
	...
	Rotation.normalize();
	Mesh->M = mat4::Translation(Position.x(), Position.y(), Position.z()) * mat4::Scale(0.5f, 0.5f, 0.5f) * mat4::RotationY(Kore::pi * 0.5f) * Rotation.matrix() * mat4::RotationX(Kore::pi * 0.5f);
}
